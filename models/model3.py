...
class model3:
    ...
    def build_model(self):
        ...
        inputs2 = Input(shape=(self.input_shape1[0],4)) # left arm
        inputs3 = Input(shape=(self.input_shape1[0],4)) # right arm
        ...
        left_arm_features = Conv1D(filters=64, kernel_size=self.input_shape1[0], activation=None, padding='valid')(inputs2)
        left_arm_features = BatchNormalization()(left_arm_features)
        left_arm_features = Activation('relu')(left_arm_features)

        right_arm_features = Conv1D(filters=64, kernel_size=self.input_shape1[0], activation=None, padding='valid')(inputs3)
        right_arm_features = BatchNormalization()(right_arm_features)
        right_arm_features = Activation('relu')(right_arm_features)

        upper_body_features = Conv1D(filters=64, kernel_size=self.input_shape1[0], activation=None, padding='valid')(inputs4)
        upper_body_features = BatchNormalization()(upper_body_features)
        upper_body_features = Activation('relu')(upper_body_features)

        lower_body_features = Conv1D(filters=64, kernel_size=self.input_shape1[0], activation=None, padding='valid')(inputs5)
        lower_body_features = BatchNormalization()(lower_body_features)
        lower_body_features = Activation('relu')(lower_body_features)

        head_features = Conv1D(filters=64, kernel_size=self.input_shape1[0], activation=None, padding='valid')(inputs6)
        ...
        conv1D = Conv1D(filters=64, kernel_size=3, activation=None, padding='same')(concatenated_features)
