...
from tensorflow.keras.layers import Input, Conv1D, LSTM, Dense, Concatenate, BatchNormalization, Activation, Dropout, Flatten
...
class model22:
    ...
    def build_model(self):
        ...
        # 4번째 1D-Convolution layer
        conv1D = Conv1D(filters=128, kernel_size=3, activation=None, padding='same')(conv1D)
        conv1D = BatchNormalization()(conv1D)
        conv1D = Activation('relu')(conv1D)

        # 5번째 1D-Convolution layer
        conv1D = Conv1D(filters=128, kernel_size=3, activation=None, padding='same')(conv1D)
        conv1D = BatchNormalization()(conv1D)
        conv1D = Activation('relu')(conv1D)

        # 6번째 1D-Convolution layer
        conv1D = Conv1D(filters=128, kernel_size=3, activation=None, padding='same')(conv1D)
        conv1D = BatchNormalization()(conv1D)
        conv1D = Activation('relu')(conv1D)
